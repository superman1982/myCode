...
-(void)setCell:(ElectronicSiteInfo *)aInfo{
    self.electronicInfo = aInfo;
    self.name.text = aInfo.name;
    [self.backgroundImageUrl setImageWithURL:[NSURL URLWithString:aInfo.backgroundImageUrl] PlaceHolder:[UIImage imageNamed:@"lvtubangretangle.png"]];
    self.description.text = aInfo.description;
    if (aInfo.routesType == atActiveRoutes) {
        ...
        self.stayTime.frame = CGRectMake(vGatherTimeFrame.origin.x, vGatherTimeFrame.origin.y + 8, vGatherTimeFrame.size.width, vGatherTimeFrame.size.height);
    }
