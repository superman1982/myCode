...
//	var contextPath = v3x.baseURL;
	...
	//换行
		...
		if(t && t.attr("comp")!= undefined){
		    ...
		    if(!v3x.isMSIE8){
		    	...
		    	if(tj.type!="fileupload"){
		    		whiteSpaceStr = ((tj.displayMode == undefined) ? whiteSpaceStr : "white-space:pre-wrap");
		    	}else{
		    		if(tj.canDeleteOriginalAtts==true){
		    			whiteSpaceStr = ((tj.displayMode == undefined) ? whiteSpaceStr : "white-space:pre-wrap");
		    		}
		    	}
		    }
	...
	var _isShowDelete = "";
	if(isShowDelete){
		_isShowDelete = "attachmentShowDelete";
	}
	str += "<div id='attachmentDiv_" + this.fileUrl2 + "' class='attachment_block "+_isShowDelete+"' style='font-size:12px; float:"+ df +formVisible+" line-height: 26px;*line-height:24px;"+whiteSpaceStr+"' "+noWrapStr+">";

	//文件类型图标
	var iconSpan='';
	if(this.type != 1 && !this.isShowImg){
	    iconSpan += '&nbsp;<span class="ico16 '+this.icon.substring(0,this.icon.indexOf('.'))+'_16 margin_r_5"></span>'  ;
//    str += "<img src='" + contextPath + "/common/images/attachmentICON/" + this.icon + "' border='0' height='16' width='16' align='absmiddle' style='margin-right: 3px;'>";
	...
	var attShowName="";
	if(this.isShowImg){
	    str += "<img style=\"CURSOR: pointer\" onclick=\"openCtpWindow({'url':$(this).attr('src')})\" src=\""+contextPath+"/fileUpload.do?method=showRTE&fileId="+this.fileUrl2+"&createDate=" + this.createDate.substring(0, 10) + "&type=image\">";
	}else if(isShowLink && (this.type == 0 || this.type == 3 || this.type == 5)){//downloadURL
	    if(this.type == 3){
	    	attNamehref += "<a onclick=\'downloadAttachment(\""+this.fileUrl2+"\",\""+this.createDate.substring(0, 10)+"\",\""+escapeStringToHTML(this.filename)+"\")\' title=\"" +escapeStringToHTML(this.filename)+ "\" target='downloadFileFrame' style='font-size:12px;color:#757575;' class='like-a'>";
	    }
	    else{
	      attNamehref += "<a href=\"" + contextPath + "/file" + (this.v?"Download":"Upload") + ".do?method=download&fileId=" + this.fileUrl2 + (this.v?("&v=" + this.v):"") + "&createDate=" + this.createDate.substring(0, 10) + "&filename=" + encodeURIComponent(this.filename) + "\" title=\"" +escapeStringToHTML(this.filename) + "\" target='downloadFileFrame' style='font-size:12px;color:#757575;'>";
	    }
	}
	if(onlineEdit && this.category != 2){
		str += "<a onclick=\"editOfficeOnline(\'"+this.id+"\')\" title=\"" + escapeStringToHTML(this.filename) + "\" target='downloadFileFrame' style='font-size:12px;color:#757575;' class='like-a'>";
		isShowLink = true;
	}
	if((this.type == 2 || this.type == 4) && this.description && (onlineEdit != false)){	//文档
		...
		attNamehref += "<a class=\"hand\" onclick=\"" + click + "\" title=\"" +escapeStringToHTML(this.filename)+ "\" style='cursor: hand;font-size:12px;'>";
		isShowLink = true;
	}
/* 对名称不再截断	
...
/*		if(width){
		...
		if(!this.isShowImg){
		    //attShowName += this.filename.getLimitLength(len).escapeHTML();
		    if(this.filename.length>len){
			attShowName += this.filename.substring(0,len).escapeHTML()+"..";
		    }else{
			attShowName += this.filename.escapeHTML();
		    }
		}
	}else{
	    if(!this.isShowImg){
		attShowName += this.filename;
	    }
	}
	
	if(this.size && this.type == 0){
	    if(!this.isShowImg){
		attShowName += "(" + (parseInt(this.size/1024) + 1) + "KB)";
	    }
	} // 名称及超链end
	...
	if(isShowLink && !this.isShowImg){
	    downloadTd= '<td align="center">'+attNamehref+'<span class="ico24 download_24"></span></a></td>';
	}
	//officetrans 查看
	if(isShowLink && this.isCanTransform &&  this.allowTrans() && this.hasSaved){
	    officetransTd += "<td align='center'><a href=\"" + contextPath + "/officeTrans.do?method=view&fileId=" + this.fileUrl2 + "&createDate=" + this.createDate.substring(0, 10) + "&filename=" + encodeURIComponent(this.filename) + "\" target='downloadFileFrame' style='font-size:12px;color:#757575;' title='"+$.i18n("officeTrans.view.label")+"'><span class='ico24 preview_24'></span></a></td>";
	}
	...
	if(seeyonProductId>0 && this.hasSaved && this.canFavourite && this.type != 2&&!this.isShowImg){
            ...
            favoriteTd +=  '<td align="center"><span id="favoriteSpan'+this.fileUrl2+'" class="ico24 unstore_24" '+sShow+' title="'+$.i18n("doc.jsp.knowledge.collection")+'" onclick="favorite(\''+ this.category + '\', \''+ this.fileUrl2 + '\',false,4)" ></span>'; 
            favoriteTd +=  '<span id="cancelFavorite'+this.fileUrl2+'" class="ico24 stored_24" '+sHide+' title="'+$.i18n("doc.jsp.cancel.collection.js")+'" onclick="cancelFavorite(\''+ this.category + '\', \''+ this.fileUrl2 + '\',false,4)" ></span></td>';
	}

	//浮动菜单
	if(!isShowDelete && !this.isShowImg && !onlineEdit && this.type != 2){
	    str += '<div class="attachment_operate">';
	        str += '<table class="attachment_operate_btn"  title="'+ escapeStringToHTML(this.filename) +'"  border="0" cellpadding="0" cellspacing="0">';
	            str += '<tr>';
	                str +=favoriteTd;
	                str +=downloadTd;
	                str += officetransTd;
	            str += '</tr>';
	        str += '</table>';
	        str += '<table class="attachment_operate_btn_bg" border="0" cellpadding="0" cellspacing="0">';
	            str += '<tr>';
	            if(favoriteTd.length>0)
	                str += '<td align="center"><div>&nbsp;</div></td>';
	            if(downloadTd.length>0)
	                str += '<td align="center"><div>&nbsp;</div></td>';
	            if(officetransTd.length>0)
	                str += '<td align="center"><div>&nbsp;</div></td>';
	            str += '</tr>';
	        str += '</table>';
	        str += '<div class="attachment_operate_bg">&nbsp;</div>';
	    str += '</div>';
		...
		//显示删除
		if(isShowDelete){
			 str += '</a><span class="ico16 affix_del_16" title="' + $.i18n("common.button.delete.label") + '" onclick="deleteAttachment(\''+ this.fileUrl2 + '\')" ></span>';       
		}
		...
		if((isShowDelete && this.canEditOnline() && this.id != undefined) && this.category == 2){
	        	 ...
	        	 str += "<span id=\"editOnline_"+this.id+"\" > <a class=\"hand\" title = \"" + $.i18n("common.toolbar.edit.label") + "\" onclick=\"editOfficeOnline4Form(\'"+this.id+"\',\'"+this.fileUrl+"\',\'"+this.filename+"\',\'"+this.mimeType+"\',\'"+this.category+"\',\'"+this.createDate+"')\"><span class=\"ico16 editor_16\"></span></a></span>";
		}	
...
function bindMouseenterFunction(){
	$(function(){
    ...
    $(".attachment_block").live("mouseenter",function() {
        $(this).find(".attachment_operate").show();
        $(this).find(".attachment_operate_bg").css("filter","alpha(opacity = 30)");
...
function editOfficeOnline4Form(id,fileUrl,filename,mimeType,category,createDate){
	...
	var url = _ctxServer+"/genericController.do?ViewPage=ctp/common/fileUpload/officeEdit&id="+id+"&content="+fileUrl+"&bodyType="+bodyType+"&createDate="+createDate+"&category="+category+"&filename="+filename;
 ...
 * 添加附件
 */	
function addAttachment(type, filename, mimeType, createDate, size, fileUrl, canDelete, needClone, description, extension, icon, reference, category, onlineView, width,hasSaved,isCanTransform,v,canFavourite,hasFavorite) {
	...
	attachment.hasFavorite = hasFavorite;
...
/*	if(!fileUploadAttachments.containsKey(fileUrl+poi)){ // 允许添加重复的关联文档这种需求有待商榷，目前尚未定论，暂时按测试的要求进行修改
  ...
  if(attachment.isShowImg){
      ...
      displayDiv.width(inpWidth-delSpanWidth).height(inpHeight);
      displayDiv.find("img").width(displayDiv.width()-delSpanWidth).height(inpHeight);
...
function editAttachments(atts,reference,subReference,category){
	...
	var result = v3x.openWindow({
		url		: getA8Top().v3x.baseURL + "/genericController.do?ViewPage=apps/collaboration/fileUpload/attEdit&category="+category+"&reference="+reference+"&subReference="+subReference+"&_isModalDialog=true",
		width	: 550,
		height	: 430,
		resizable	: "yes"
	});
 ...
 * 转换XSL样式
 */
function getXSLDoc(xmlDoc, xsltDoc) 
{
    // 1.
...
MxtWindow.prototype.getReturnValue = function(args){
	...
	if(win!=null){
		...
		if(win.contentWindow){
			if (typeof(args) != 'undefined' && args != null) { 
				returnValues =win.contentWindow.OK(args); 
			} else { 
				returnValues =win.contentWindow.OK(); 
			}
		}else{
			if (typeof(args) != 'undefined' && args != null) {
				returnValues =win.OK(args); 
			} else { 
				returnValues =win.OK(); 
			}
		}
...
MxtWindow.prototype.getPanel = function(){
    ...
    if (this.isIframe) {
        ...
        ifram.style.height = parseInt(body.style.height)- 0 + "px";
        ifram.style.width = this.width - 0 + "px";
        this.iframe = ifram;
...
MxtWindow.prototype.getWindow = function(){
    ...
    body.style.height = this.height - this.titleHeight - this.footerHeight - 0 + "px";
