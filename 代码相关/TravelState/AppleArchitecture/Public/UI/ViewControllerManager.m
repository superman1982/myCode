...
static NSMutableDictionary *sViewControllerHashMap = nil;
// 当前显示的BaseViewController
static BaseViewController *sCurrBaseViewController = nil;
//加入到堆栈中的所有VC名字
static NSMutableArray *sVCNameArray = nil;
// 增加BaseViewController到字典中
+ (void) addBaseViewController: (BaseViewController *) aBaseViewController {
...
+ (void) showBaseViewController: (NSString *) aBaseViewControllerName AnimationType:(ViewAnnatation )aType SubType:(ViewAnnatationSubtype)aSubType{
    if ([AppDelegateInstance navigationController] == nil) {
        return;
    }
    ...
    [ViewControllerManager showAnimationViewController:vBaseViewController AnimationType:aType SubType:aSubType];
...
+(void)showAnimationViewController:(BaseViewController *) aBaseViewControllerName AnimationType:(ViewAnnatation )aType SubType:(ViewAnnatationSubtype)aSubType{
    if (aType != vaDefaultAnimation && aType != vaNoAnimation) {
        //自定义动画
        [[AppDelegateInstance navigationController] pushViewController:aBaseViewControllerName animated:NO];
        [UIView animateChangeView:[AppDelegateInstance navigationController].view AnimationType:aType SubType:aSubType Duration:.4 CompletionBlock:^{}];
    }else if(aType == vaDefaultAnimation){
        // 默认动画
        [[AppDelegateInstance navigationController] pushViewController: aBaseViewControllerName animated: YES];
...
+(void)showPopAnimation:(ViewAnnatation)aViewType SubType:(ViewAnnatationSubtype)aSubType
{
    ...
    if (aViewType != vaDefaultAnimation && aViewType != vaNoAnimation) {
        //自定义动画
        [UIView animateChangeView:[AppDelegateInstance navigationController].view AnimationType:aViewType SubType:aSubType Duration:.3 CompletionBlock:nil];
        [[AppDelegateInstance navigationController] popViewControllerAnimated:NO];
    }else if(aViewType == vaDefaultAnimation){
        //默认动画
        [[AppDelegateInstance navigationController] popViewControllerAnimated:YES];
...
{
    BaseViewController *vTopVC = (BaseViewController *)[AppDelegateInstance navigationController].topViewController;
    //从字典中移除不需要的VC
    [ViewControllerManager removeBaseViewController:[NSStringFromClass([vTopVC class]) lowercaseString]];
    [sVCNameArray removeObject:[NSStringFromClass([vTopVC class]) lowercaseString]];
    //设置返回动画
    [ViewControllerManager showPopAnimation:aViewType SubType:aSubType];
    //重新改变当前显示的VC
    sCurrBaseViewController = (BaseViewController *)[AppDelegateInstance navigationController].topViewController;
}
...
+(void)backToViewController:(NSString *)aBaseViewControllerName Animatation:(ViewAnnatation)aViewType SubType:(ViewAnnatationSubtype)aSubType{
    ...
    if (aViewType != vaDefaultAnimation && aViewType != vaNoAnimation) {
        //移除时加载的动画
        [UIView animateChangeView:[AppDelegateInstance navigationController].view AnimationType:aViewType SubType:aSubType Duration:.3 CompletionBlock:nil];
        [[AppDelegateInstance navigationController] popToViewController:vNeedRemovedVC animated:NO];
    }else if(aViewType == vaDefaultAnimation){
        [[AppDelegateInstance navigationController]  popToViewController:vNeedRemovedVC animated:YES];
    }else if (aViewType == vaNoAnimation){
        [[AppDelegateInstance navigationController]  popToViewController:vNeedRemovedVC animated:NO];
    }
    //移除掉字典中的VC
