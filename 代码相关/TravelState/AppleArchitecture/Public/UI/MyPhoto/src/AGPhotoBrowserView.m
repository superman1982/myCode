...
- (UIButton *)doneButton
{
	if (!_doneButton) {
		...
		[_doneButton setTitle:NSLocalizedString(@"完成", @"Title for Done button") forState:UIControlStateNormal];
...
- (void)configureCell:(UITableViewCell<AGPhotoBrowserCellProtocol> *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([_dataSource respondsToSelector:@selector(photoBrowser:URLStringForImageAtIndex:)] && [cell respondsToSelector:@selector(setCellImageWithURL:)]) {
        PhtotoInfo *vPhtotInfo = [_dataSource photoBrowser:self URLStringForImageAtIndex:indexPath.row];
        if (vPhtotInfo.phtotURLStr == Nil) {
            [cell setCellImage:vPhtotInfo.photoImage];
        }else{
            [cell setCellImageWithURL:[NSURL URLWithString:vPhtotInfo.phtotURLStr]];
        }
    }
}
		...
		if (heightDifference <= AGPhotoBrowserThresholdToCenter) {
			...
			[self hideWithCompletion:^(BOOL finished){
				...
				if (strongSelf) {
                    ...
                    if ([_delegate respondsToSelector:@selector(photoBrowserdidPanMoved:)]) {
                        [_delegate photoBrowserdidPanMoved:self];
                    }
				}
			}];
		}
